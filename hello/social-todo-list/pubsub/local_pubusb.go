...
// It has a queue (buffer channel) at its core and many group of subscribers.
...
func (ps *localPubSub) Publish(ctx context.Context, topic Topic, data *Message) error {
	...
	go func() {
		...
		log.Println("v...: New message published:", data.String())
	}()

	return nil
}

func (ps *localPubSub) Subscribe(ctx context.Context, topic Topic) (<-chan *Message, func()) {
	c := make(chan *Message)
...
func (ps *localPubSub) run() error {
	go func() {
		...
		for {
			...
			log.Println("v...: Message dequeue:", mess.String())
...
func (s *localPubSub) Stop() <-chan bool {
	c := make(chan bool)
	go func() {
		c <- true
	}()
