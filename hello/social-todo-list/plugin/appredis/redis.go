...
func (r *redisDB) InitFlags() {
	...
	flag.StringVar(&r.RedisUri, prefix+"uri", "redis://localhost:6379", " (For go-redis) Redis connection-string. Ex: redis://user:pass@host:port/db")
	flag.IntVar(&r.MaxActive, prefix+"pool-max-active", defaultRedisMaxActive, " (For go-redis) Override redis pool MaxActive")
	flag.IntVar(&r.MaxIdle, prefix+"pool-max-idle", defaultRedisMaxIdle, " (For go-redis) Override redis pool MaxIdle")
}
...
func (r *redisDB) Configure() error {
	...
	r.logger.Info("Connecting to Redis at %s", r.RedisUri)

	opt, err := redis.ParseURL(r.RedisUri)
	if err != nil {
		r.logger.Error("Cannot parse Redis URI: %s", err.Error())
		return err
	}
	...
	if err := client.Ping().Err(); err != nil {
		r.logger.Error("Cannot connect Redis: %s", err.Error())
		return err
	}
...
func (r *redisDB) Stop() <-chan bool {
	if r.client != nil {
		if err := r.client.Close(); err != nil {
			r.logger.Info("cannot close %s: %s", r.name, err.Error())
		}
	}
